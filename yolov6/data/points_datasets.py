    ...
    def __getitem__(self, index):
        ...
        if self.augment and random.random() < self.hyp["mosaic"]:
            ...
            if labels.size:
                ...
                for i in range(4, NUMPOINT*2+4,2):
                    boxes[:, i] = np.array(labels[:, i+1] > 0 ,dtype=np.int32) * (w * labels[:, i+1] + pad[0]) + (np.array(labels[:, i+1] > 0 , dtype=np.int32) -1 )
                    boxes[:, i+1] = np.array(labels[:, i+2] > 0 ,dtype=np.int32) * (h * labels[:, i+2] + pad[1]) + (np.array(labels[:, i+2] > 0 , dtype=np.int32) -1 )
        ...
        if len(labels):
            ...
            odd_index = list(range(5,NUMPOINT*2+5,2))
            even_index = list(range(6,NUMPOINT*2+5,2))
            labels[:, odd_index] /= img.shape[1]  # normalized landmark x 0-1
            labels[:, odd_index] = np.where(labels[:, odd_index] < 0, -1, labels[:, odd_index])
            labels[:, even_index] /= img.shape[0]  # normalized landmark y 0-1
            labels[:, even_index] = np.where(labels[:, even_index] < 0, -1, labels[:, even_index])
        #self.showlabels(img,labels[:,1:5],labels[:,5:15])
        if self.augment:
            img, labels = self.general_augment(img, labels.copy())

        labels_out = torch.zeros((len(labels), (NUMPOINT*2+6)))
    ...
    def check_label_files(args):
        ...
        try:
            if osp.exists(lb_path):
                ...
                if len(labels):
                    assert all(
                        len(l) == (NUMPOINT*2+5) for l in labels
