...
class ComputeLoss:
    ...
    def preprocess(self, targets, batch_size, scale_tensor):
        targets_list = np.zeros((batch_size, 1, (NUMPOINT*2+5))).tolist()
        for i, item in enumerate(targets.cpu().numpy().tolist()):
            targets_list[int(item[0])].append(item[1:])
        max_len = max((len(l) for l in targets_list))
        marks = [-1] * (NUMPOINT * 2)
        targets = torch.from_numpy(np.array(list(map(lambda l:l + [[-1,0,0,0,0]+marks]*(max_len - len(l)), targets_list)))[:,1:,:]).to(targets.device)
        
        batch_target = targets[:, :, 1:5].mul_(scale_tensor)
        targets[..., 1:5] = xywh2xyxy(batch_target)

        # landmarks
        gt_ldmks = targets[:, :, 5:].mul_(scale_tensor[0,0])
        return targets, gt_ldmks

    def bbox_decode(self, anchor_points, pred_dist):
        pred_distri = pred_dist.clone()[..., :-(NUMPOINT*2)]
        pred_ldmks = pred_dist.clone()[..., -(NUMPOINT*2):]
        if self.use_dfl:
        ...
        pred_ldmks[..., 0:(NUMPOINT*2):2] += torch.unsqueeze(anchor_points[..., 0:1], 0).repeat(pred_dist.shape[0],1,NUMPOINT)
        pred_ldmks[..., 1:(NUMPOINT*2):2] += torch.unsqueeze(anchor_points[..., 1:2], 0).repeat(pred_dist.shape[0],1,NUMPOINT)
...
class LandmarksLoss(nn.Module):
    ...
    def forward(self, pred_ldmks, gt_ldmks, mask):
        mask = mask.unsqueeze(-1).repeat([1, 1, (NUMPOINT*2)])
        pred_ldmks_pos = torch.masked_select(pred_ldmks, mask).reshape([-1, (NUMPOINT*2)])
        gt_ldmks_pos = torch.masked_select(gt_ldmks, mask).reshape([-1, (NUMPOINT*2)])
